...
if subj in ['S111', 'S114']:
    kernels = ['rbf']
else:
    kernels = ['linear', 'rbf']
estimators = [('svm', SVC(probability=True, random_state = 42, class_weight = 'balanced'))] #('rf', RandomForestClassifier(random_state = 42))]
param_grid = dict(
    svm__C = [.1, .5, 1, 5, 10, 50, 100],
...
with open(f'reports_clean/{subj}_run{run}_SVM_clean_pca.txt', 'w') as f:
